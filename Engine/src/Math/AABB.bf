...
namespace SteelEngine
{
	...
	struct AABB
	{
		...
		[Inline] public float Area => default;
		[Inline] public bool HasNoArea => (size.x <= 0 || size.y <= 0 || size.z <= 0);
		...
		[Inline]
		bool IsEqualApprox(Self other) => Vector3.IsEqualsApprox(position, other.position) && Vector3.IsEqualsApprox(size, other.size);
		...
		Self Merge(Self with) { return default; }
		...
		//bool IntersectsConvexShape(const Plane *p_planes, int p_plane_count, const Vector3 *p_points, int p_point_count) const;
		//bool inside_convex_shape(const Plane *p_planes, int p_plane_count) const;
		...
		bool has_point(Vector3 point) { return default; }
		Vector3 get_support(Vector3 normal) { return default; }
